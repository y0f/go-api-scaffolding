...
func run() error {
	...
	widgetHandler := widget.NewHandler(
		...
		idempotency.NewStore(pool, 24*time.Hour),
	)
	...
	// Wait for the outbox relay to stop before the deferred pool.Close runs, so
	// it cannot run a query against a closed pool. Bound the wait so a stuck
	// relay cannot block shutdown indefinitely.
	select {
	case <-relayDone:
